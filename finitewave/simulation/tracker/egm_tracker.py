...
class EGMTracker(Tracker):
    ...
    def initialize(self, simulation):
        ...
        self.lead_fields = self.simulation.backend.wrap_array(np.array(self.lead_fields))
    ...
    def calc_egm(self):
        ...
        """
        u_old = self.simulation.time_integration.u_old
        u = self.simulation.cardiac_model._u
        rhs = self.simulation.cardiac_model._rhs
...
def egm_func(backend):
    if backend.name == "numba":
        ...
        def calc_egm_numba(u, u_old, rhs, dt, lead_field):
            ...
            for i in range(n):
                egm = 0.0
                for j in prange(tr_current.shape[0]):
                    egm += tr_current[j] * lead_field[i, j]
