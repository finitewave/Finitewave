...
class ImplicitSolver(SolverBase):
    ...
    def initialize(self, simulation):
        ...
        self.m_rhs = simulation.backend.wrap(0. * self.u)
    ...
    def select_method(self, backend):
        if backend.name == "numba":
            from .numba_linalg.numba_linalg import NumbaLinalg
            self.linalg = NumbaLinalg()
            return

        if backend.name == "mlx":
            from .mlx_linalg.mlx_linalg import MlxLinalg
            self.linalg = MlxLinalg()
            return
        
        if backend.name == "jax":
            from .jax_linalg.jax_linalg import JaxLinalg
    ...
    def assemble_system(self):
        ...
        self.a_lhs_matrix = self.linalg.wrap_matrix(a_lhs_matrix, dtype)
        self.a_rhs_matrix = self.linalg.wrap_matrix(a_rhs_matrix, dtype)
        self.a_ion_matrix = self.linalg.wrap_matrix(a_ion_matrix, dtype)
    ...
    def run(self):
        ...
        # b = A_ion @ rhs
        self.b = self.linalg.matvec(*self.a_ion_matrix, self.rhs, self.myo_indexes, self.b)
        # b = A_rhs @ u_old + 1. * b
        self.b = self.linalg.explicit_step(*self.a_rhs_matrix, self.u_old, self.b,
                                                  1., self.myo_indexes, self.b)
        # Better initial guess for CG solver using the previous two time steps
        self.u = self.linalg.axmy(2., self.u_old, self.u_old_2,
                                         self.myo_indexes, self.u)
        self.u, n_iter = self.linalg.solve(*self.a_lhs_matrix, self.b, 
...
class CrankNicolsonSolver(ImplicitSolver):
    def __init__(self, atol=1e-8, maxiter=100, lumping_factor=0.0, full_lumping=False):
...
class BackwardEulerSolver(ImplicitSolver):
    def __init__(self, atol=1e-8, maxiter=100, lumping_factor=0.0, full_lumping=False):
        super().__init__(atol=atol, maxiter=maxiter, lumping_factor=lumping_factor, 
                         order=1, full_lumping=full_lumping)
