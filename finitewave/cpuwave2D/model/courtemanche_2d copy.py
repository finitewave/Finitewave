...
class Courtemanche2D(CardiacModel):
    ...
    def run_ionic_kernel(self):
        ...
        variables
        """
        ionic_kernel_2d(self.u_new, self.u, self.nai, self.nao, self.ki, self.ko, 
                        self.cai, self.cao, self.m, self.h, self.j_, self.d, self.f,
                        self.xs, self.xr, self.ato, self.iito, self.uakur, self.uikur, 
                        self.fca, self.ireljsrol,
...
@njit
def calc_ina(v, nai, nao, dt, m, h, j, R, temp, frdy):
    gna = 7.8
...
@njit
def calc_ical(v, cai, dt, d, f, fca, gcalbar):
...
@njit
def calc_ikr(v, ko, ki, dt, xr, R, temp, frdy):
...
@njit
def calc_iks(v, ko, ki, dt, xs, R, temp, frdy):
    gks = 0.129
...
@njit
def calc_iki(v, ko, ki, R, temp, frdy, dt):
    gki = 0.09 * (ko / 5.4) ** 0.4
...
# @njit
def calc_ikach(v, ko, ki, dt, yach, ach, R, temp, frdy):
    gkach = 0.135
    ekach = ((R * temp) / frdy) * np.log(ko / ki)
    if ach > 0:
        alphayach = 1.232e-2 / (1 + 0.0042 / ach) + 0.0002475
    else:
        alphayach = 0.0  # No acetylcholine effect
...
@njit
def calc_ikur(v, ko, ki, dt, uakur, uikur, R, temp, frdy):
...
@njit
def calc_ito(v, ko, ki, dt, ato, iito, R, temp, frdy):
    gito = 0.1652
...
@njit
def calc_inaca(v, nai, nao, cai, cao, R, temp, frdy, gammas, kmnancx, kmcancx, ksatncx):
...
@njit
def calc_inak(v, nai, ko, nao, ibarnak, kmnai, kmko):
...
@njit
def calc_ipca(cai, ibarpca, kmpca):
    ipca = (ibarpca * cai) / (kmpca + cai)
    return ipca

@njit
def calc_icab(v, cai, cao, R, temp, frdy):
    gcab = 0.00113
...
@njit
def calc_inab(v, nai, nao, R, temp, frdy):
    gnab = 0.000674
...
# tp06 epi kernel
# @njit(parallel=True)
def ionic_kernel_2d(u_new, u, nai, nao, ki, ko, cai, cao, m, h, j_, d, f, xs, xr, ato, iito, uakur, uikur, fca, ireljsrol, indexes, dt):
    n_i = u.shape[0]
    n_j = u.shape[1]
    ...
    for ind in prange(indexes.shape[0]):
        ...
        ina, m[i, j], h[i, j], j_[i, j] = calc_ina(u[i, j], nai[i, j], nao[i, j], dt, m[i, j], h[i, j], j_[i, j], R, temp, frdy)
        ilca, d[i, j], f[i, j], fca[i, j] = calc_ical(u[i, j], cai[i, j], dt, d[i, j], f[i, j], fca[i, j], gcalbar)
        ikr, xr[i, j] = calc_ikr(u[i, j], ko[i, j], ki[i, j], dt, xr[i, j], R, temp, frdy)
        iks, xs[i, j] = calc_iks(u[i, j], ko[i, j], ki[i, j], dt, xs[i, j], R, temp, frdy)
        iki = calc_iki(u[i, j], ko[i, j], ki[i, j], R, temp, frdy, dt)
        ikach, yach = calc_ikach(u[i, j], ko[i, j], ki, dt, yach, ach, R, temp, frdy)
        ikur, uakur[i, j], uikur[i, j] = calc_ikur(u[i, j], ko[i, j], ki[i, j], dt, uakur[i, j], uikur[i, j], R, temp, frdy)
        ito, ato[i, j], iito[i, j] = calc_ito(u[i, j], ko[i, j], ki[i, j], dt, ato[i, j], iito[i, j], R, temp, frdy)
        inaca = calc_inaca(u[i, j], nai[i, j], nao[i, j], cai[i, j], cao[i, j], R, temp, frdy, gammas, kmnancx, kmcancx, ksatncx)
        inak = calc_inak(u[i, j], nai[i, j], ko[i, j], nao[i, j], ibarnak, kmnai, kmko)
        ipca = calc_ipca(cai[i, j], ibarpca, kmpca)
        icab = calc_icab(u[i, j], cai[i, j], cao[i, j], R, temp, frdy)
        inab = calc_inab(u[i, j], nai[i, j], nao[i, j], R, temp, frdy)

        caiont = ilca+icab+ipca-2*inaca

        nai[i, j] = conc_nai(nai[i, j], ina + inab + 3 * inak + 3 * inaca, acap, vmyo, zna, frdy, dt)
        ki[i, j] = conc_ki(ki[i, j], iki + ikr + iks + ikach + ikur + ito - 2 * inak, acap, vmyo, zk, frdy, dt)
        itr = calc_itr(nsr, jsr, tautr)
        nsr, ileak, iup = conc_nsr(nsr, itr, iupbar, nsrbar, cai[i, j], kmup, vjsr, vnsr, dt)
        jsr, urel, vrel, wrel = conc_jsr(jsr, itr, ireljsrol[i, j], vjsr, caiont, acap, frdy, grelbarjsrol, cai[i, j], u[i, j], csqnbar, kmcsqn, dt)
        cai[i, j] = conc_cai(cai[i, j], caiont, dt, ileak, iup, ireljsrol[i, j], trpnbar, kmtrpn, cmdnbar, kmcmdn, acap, frdy, vmyo, vnsr, vjsr)
