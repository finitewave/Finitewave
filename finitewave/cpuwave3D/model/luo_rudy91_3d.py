...
def ionic_kernel_3d(u_new, u, m, h, j_, d, f, x, Cai_c, indexes, dt):
    ...
    for ni in prange(len(indexes)):
        ...
        I_Na = 23 * pow(m[i, j, k], 3) * h[i, j, k] * j_[i, j, k] * (u[i, j, k] - E_Na)

        alpha_h, beta_h, beta_J, alpha_J = 0, 0, 0, 0
        if u[i, j, k] >= -40.:
            beta_h = 1. / (0.13 * (1 + np.exp((u[i, j, k] + 10.66) / -11.1)))
            beta_J = 0.3 * np.exp(-2.535 * 1e-07 *
                                  u[i, j, k]) / (1 + np.exp(-0.1 * (u[i, j, k] + 32)))
        else:
            alpha_h = 0.135 * np.exp((80 + u[i, j, k]) / -6.8)
            beta_h = 3.56 * \
                np.exp(0.079 * u[i, j, k]) + 3.1 * 1e5 * np.exp(0.35 * u[i, j, k])
            beta_J = 0.1212 * \
                np.exp(-0.01052 * u[i, j, k]) / \
                (1 + np.exp(-0.1378 * (u[i, j, k] + 40.14)))
            alpha_J = (-1.2714 * 1e5 * np.exp(0.2444 * u[i, j, k]) - 3.474 * 1e-5 * np.exp(-0.04391 * u[i, j, k])) * \
                      (u[i, j, k] + 37.78) / (1 + np.exp(0.311 * (u[i, j, k] + 79.23)))

        alpha_m = 0.32 * (u[i, j, k] + 47.13) / \
            (1 - np.exp(-0.1 * (u[i, j, k] + 47.13)))
        ...
        alpha_d = 0.095 * \
            np.exp(-0.01 * (u[i, j, k] - 5)) / \
            (1 + np.exp(-0.072 * (u[i, j, k] - 5)))
        beta_d = 0.07 * \
            np.exp(-0.017 * (u[i, j, k] + 44)) / \
            (1 + np.exp(0.05 * (u[i, j, k] + 44)))
        alpha_f = 0.012 * \
            np.exp(-0.008 * (u[i, j, k] + 28)) / \
            (1 + np.exp(0.15 * (u[i, j, k] + 28)))
        beta_f = 0.0065 * \
            np.exp(-0.02 * (u[i, j, k] + 30)) / \
            (1 + np.exp(-0.2 * (u[i, j, k] + 30)))
        ...
        alpha_x = 0.0005 * \
            np.exp(0.083 * (u[i, j, k] + 50)) / \
            (1 + np.exp(0.057 * (u[i, j, k] + 50)))
        beta_x = 0.0013 * \
            np.exp(-0.06 * (u[i, j, k] + 20)) / \
            (1 + np.exp(-0.04 * (u[i, j, k] + 20)))
        ...
        beta_K1 = (0.49124 * np.exp(0.08032 * (u[i, j, k] - E_K1 + 5.476)) + np.exp(0.06175 * (u[i, j, k] - E_K1 - 594.31))) / \
                  (1 + np.exp(-0.5143 * (u[i, j, k] - E_K1 + 4.753)))
