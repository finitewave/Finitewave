...
from ..stencil2D.asymmetric_stencil_2d import (
    AsymmetricStencil2D,
    major_component,
    minor_component
)
...
class AsymmetricStencil3D(AsymmetricStencil2D):
    ...
    def diffusion_kernel(self, u_new, u, rhs, w, indexes):
        """
        ...
        """
        return diffusion_kernel_3d_aniso(u_new, u, rhs, w, indexes)
    ...
    def compute_weights(self, simulation):
        ...
        mesh = simulation.cardiac_tissue.mesh.copy()
        mesh[mesh != 1] = 0
        conductivity = simulation.cardiac_tissue.conductivity
        conductivity = conductivity * np.ones_like(mesh,
                                                   dtype=simulation.npfloat)

        fibers = simulation.cardiac_tissue.fibers
        ...
        weights = np.zeros((*mesh.shape, 19), dtype=simulation.npfloat)
        weights = compute_weights(weights, mesh, d_xx, d_xy, d_xz, d_yx, d_yy,
                                  d_yz, d_zx, d_zy, d_zz)

        weights = weights * d_model * dt / dr**2
        weights[:, :, :, 4] += 1
        return weights


@njit(parallel=True)
def diffusion_kernel_3d_aniso(u_new, u, rhs, w, indexes):
    """
    ...
    rhs : numpy.ndarray
        A 3D array representing the right-hand side of the diffusion equation.
    w : numpy.ndarray
        Array of weights for diffusion, with the shape of (*mesh.shape, 19).
    indexes : numpy.ndarray
        Array of indices where the diffusion should be computed.
    ...
    for ind in prange(len(indexes)):
        ...
        u_new[i, j, k] = (u[i-1, j-1, k] * w[i, j, k, 0] +
                          ...
                          u[i+1, j, k+1] * w[i, j, k, 18]) + rhs[i, j, k]
    return u_new
