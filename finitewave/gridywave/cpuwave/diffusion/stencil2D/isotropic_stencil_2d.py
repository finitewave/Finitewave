...
class IsotropicStencil2D(Stencil):
    ...
    def diffusion_kernel(self, u_new, u, rhs, weights, indexes):
        """
...
@njit(parallel=True)
def diffusion_kernel(u_new, u, rhs, w, indexes):
    """
...
@njit(parallel=True)
def compute_weights(w, m, d_xx, d_yy, indexes):
    """
    Computes the weights for isotropic diffusion in 2D.
