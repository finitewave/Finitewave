...
class CrankNicolsonTimeIntegration(ImplicitTimeIntegration):
    """Advance cardiac simulations with Crank-Nicolson time integration.

    Diffusion uses equal weights for the old and new solution. The reaction
    contribution is supplied explicitly by the cardiac model, and the
    resulting linear system is solved with Conjugate Gradient by default.
    ...
    ----------
    order : int
        Diffusion method order, fixed to 2.
    ...
    -----
    Each step solves
    ``(M_eff + 0.5 * dt * K) @ u_new = (M_eff - 0.5 * dt * K) @ u_old + dt * M_r @ f``,
    ...
    The diffusion discretization is second order in time. The accuracy of the
    full reaction-diffusion simulation also depends on the cardiac model's
    reaction update; this integrator does not time-average the reaction term.
    ...
    >>> sim.time_integration = fw.CrankNicolsonTimeIntegration(atol=1e-6)
    ...
    def __init__(self, atol=1e-8, maxiter=100, lumping_factor=0.0,
        ...
        super().__init__(atol=atol, maxiter=maxiter, lumping_factor=lumping_factor,
                         order=2, reaction_lumping=reaction_lumping)
