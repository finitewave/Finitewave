...
from .linalg.mlx_solvers import (
    matvec_mlx,
    cg_mlx
)
...
class CrankNicolsonCGSolver(Solver):
    ...
    def initialize(self, simulation):
        ...
        self.num_iterations = []
        self.b = mx.zeros_like(simulation.cardiac_model.u)
        self.u = simulation.cardiac_model.u
        self.u_old = mx.array(self.u, dtype=mx.float32)
    ...
    def assemble_system(self):
        ...
        self.a_lhs_matrix = self.build_ellpack(mass + 0.5 * dt * stiff)
        self.a_rhs_matrix = self.build_ellpack(mass - 0.5 * dt * stiff)
    ...
    def run(self):
        ...
        self.u = self.u_old + self.simulation.dt * self.rhs
        self.b = matvec_mlx(*self.a_rhs_matrix, self.u, self.myo_indexes)

        self.u, success = cg_mlx(*self.a_lhs_matrix, self.b, self.u,
                                 self.myo_indexes, atol=self.atol, maxiter=self.maxiter)
        if success < 0:
