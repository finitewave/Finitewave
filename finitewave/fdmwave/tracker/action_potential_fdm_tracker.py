...
class ActionPotentialFDMTracker(Tracker):
    """
    ...
    def __init__(self):
        """
        Initializes the ActionPotentialFDMTracker with default parameters.
        """
        Tracker.__init__(self)
        self.act_pot = []  # Initialize the array to store action potential
        self.default_cell_ind = [1, 1, 1]       # Default cell indices to track
        self.cell_ind = None
    ...
    def initialize(self, model):
        ...
        if self.cell_ind is None:
            self.cell_ind = self.default_cell_ind

            if model.cardiac_tissue.mesh.ndim == 2:
                self.cell_ind = self.default_cell_ind[:2]
