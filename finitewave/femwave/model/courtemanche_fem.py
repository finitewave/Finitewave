...
from .cardiac_model_fem import CardiacModelFEM


class CourtemancheFEM(CardiacModelFEM):
    """
    ...
    def initialize(self):
        ...
        """
        super().initialize()                                      # (mV)
        self.nai = self.init_nai * np.ones_like(self.u)     # (mM)
        self.ki = self.init_ki * np.ones_like(self.u)       # (mM)
        self.cai = self.init_cai * np.ones_like(self.u)     # (mM)
        self.caup = self.init_caup * np.ones_like(self.u)   # (mM)
        self.carel = self.init_carel * np.ones_like(self.u) # (mM)
        self.m = self.init_m * np.ones_like(self.u)    
        self.h = self.init_h * np.ones_like(self.u)      
        self.j_ = self.init_j * np.ones_like(self.u)
        self.d = self.init_d * np.ones_like(self.u)
        self.f = self.init_f * np.ones_like(self.u)
        self.oa = self.init_oa * np.ones_like(self.u)
        self.oi = self.init_oi * np.ones_like(self.u)
        self.ua = self.init_ua * np.ones_like(self.u)
        self.ui = self.init_ui * np.ones_like(self.u)
        self.xs = self.init_xs * np.ones_like(self.u)
        self.xr =self.init_xr * np.ones_like(self.u)
        self.fca = self.init_fca * np.ones_like(self.u)
        self.irel = self.init_irel * np.ones_like(self.u)
        self.vrel = self.init_vrel * np.ones_like(self.u)
        self.urel = self.init_urel * np.ones_like(self.u)
        self.wrel = self.init_wrel * np.ones_like(self.u)
    ...
    def run_ionic_kernel(self):
        ...
        variables
        """
        ionic_kernel_elems(self._u, self._rhs, self.nai, self.ki, self.cai, self.caup, self.carel, 
...
@njit(parallel=True)
def ionic_kernel_elems(u, rhs, nai, ki, cai, caup, carel, m, h, j_, d, f, oa, oi, ua, ui, xs, xr, fca, irel, vrel, urel, wrel, indexes, dt, 
    ...
    for i in prange(indexes.shape[0]):
        ...
        ena, ek, eca = calc_equilibrum_potentials(nai[ind], nao, ki[ind], ko, cai[ind], cao, R, T, F)

        m[ind] = calc_gating_m(m[ind], u[i], dt)
        h[ind] = calc_gating_h(h[ind], u[i], dt)
        j_[ind] = calc_gating_j(j_[ind], u[i], dt)

        ina = calc_ina(u[i], m[ind], h[ind], j_[ind], gna, ena)

        ik1 = calc_ik1(u[i], gk1, ek)

        ito, oa[ind], oi[ind] = calc_ito(u[i], dt, kq10, oa[ind], oi[ind], gto, ek)

        ikur, ua[ind], ui[ind] = calc_ikur(u[i], dt, kq10, ua[ind], ui[ind], ek, gkur_coeff)

        ikr, xr[ind] = calc_ikr(u[i], dt, xr[ind], gkr, ek)

        iks, xs[ind] = calc_iks(u[i], dt, xs[ind], gks, ek)

        ical, d[ind], f[ind], fca[ind] = calc_ical(u[i], dt, d[ind], f[ind], cai[ind], gcal, fca[ind], eca)

        inak = calc_inak(inakmax, nai[ind], nao, ko, kmnai, kmko, F, u[i], R, T)
        inaca = calc_inaca(inacamax, nai[ind], nao, cai[ind], cao, kmnancx, kmcancx, ksatncx, F, u[i], R, T)

        ibca = calc_ibca(gcab, eca, u[i])

        ibna = calc_ibna(gnab, ena, u[i])

        ipca = calc_ipca(ipcamax, cai[ind])

        irel[ind], urel[ind], vrel[ind], wrel[ind] = calc_irel(dt, urel[ind], vrel[ind], irel[ind], wrel[ind], ical, inaca, krel, carel[ind], cai[ind], u[i], F, Vrel)
        itr = calc_itr(caup[ind], carel[ind])
        iup = calc_iup(iupmax, cai[ind], kup)
        iupleak = calc_iupleak(caup[ind], caupmax, iupmax) 

        caup[ind] = calc_caup(caup[ind], dt, iup, iupleak, itr, Vrel, Vup)
        nai[ind] = calc_nai(nai[ind], dt, inak, inaca, ibna, ina, F, Vj)

        ki[ind] = calc_ki(ki[ind], dt, inak, ik1, ito, ikur, ikr, iks, ibk, F, Vj)
        cai[ind] = calc_cai(cai[ind], dt, inaca, ipca, ical, ibca, iup, iupleak, irel[ind], Vrel, Vup, trpnmax, kmtrpn, cmdnmax, kmcmdn, F, Vj)

        carel[ind] = calc_carel(carel[ind], dt, itr, irel[ind], csqnmax, kmcsqn)

        rhs[i] = - dt * (ina + ik1 + ito + ikur + ikr + iks + ical + ipca + inak + inaca + ibna + ibca)
